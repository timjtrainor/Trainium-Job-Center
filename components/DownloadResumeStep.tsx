...
const transformAiWorkflowResume = (aiResume: AiWorkflowResume): Resume => {
  ...
  return {
    ...
    work_experience,
    education,
    certifications: [], // Add missing certifications field
    skills,
};
...
const normalizeResume = (resume: Resume | AiWorkflowResume): Resume => {
  ...
  if (format === 'ai-workflow') {
    const transformed = transformAiWorkflowResume(resume as AiWorkflowResume);
    return transformed;
  }

  // Handle standard resume format and normalize education structure
  const standardResume = resume as Resume;
  const normalized = { ...standardResume };
...
const generatePdf = (resume: Resume | AiWorkflowResume, companyName: string) => {
    ...
    const { header, summary, work_experience, education, skills } = normalizedResume;
