...
export const ApplicationsTable = ({ title, applications, companies, statuses, strategicNarratives, onViewApplication, onViewCompany, onUpdateApplicationStatus, onDeleteApplication }: ApplicationsTableProps): React.ReactNode => {
  ...
  const companyResumeCounts = useMemo(() => {
    const counts = new Map<string, number>();
    const targetStatuses = ['Step-3: Resume created', 'Step-4: Applied', 'Waiting', 'Interviewing'];

    applications.forEach(app => {
      if (app.status && targetStatuses.includes(app.status.status_name)) {
        const currentCount = counts.get(app.company_id) || 0;
        counts.set(app.company_id, currentCount + 1);
      }
    });
    return counts;
  }, [applications]);
  ...
  }, [applications, searchTerm, statusFilter, showRejected, companyMap]);
      ...
      <div className="p-4 sm:p-6 border-b border-slate-200 dark:border-slate-700">
        ...
        <div className="mt-4 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="w-full sm:w-1/2">
            <input
              type="text"
              placeholder="Search by title, company, or track..."
              value={searchTerm}
        ...
        <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
          <thead className="bg-slate-50 dark:bg-slate-800">
            <tr>
              ...
              <th scope="col" className="px-3 py-3.5 text-left text-sm font-semibold text-gray-900 dark:text-white">Track</th>
              ...
              <th scope="col" className="px-3 py-3.5 text-left text-sm font-semibold text-gray-900 dark:text-white">Resume Ver.</th>
              <th scope="col" className="px-3 py-3.5 text-left text-sm font-semibold text-gray-900 dark:text-white">Co. Resumes</th>
          ...
          <tbody className="divide-y divide-gray-200 dark:divide-slate-700 bg-white dark:bg-slate-800">
            {filteredAndSortedApplications.map(app => (
              <tr key={app.job_application_id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50 group">
                ...
                <td className="px-3 py-4 text-sm text-gray-500 dark:text-slate-400 cursor-pointer" onClick={() => onViewApplication(app.job_application_id)}>{app.job?.track || 'N/A'}</td>
                ...
                <td className="px-3 py-4 text-sm text-gray-500 dark:text-slate-400 cursor-pointer text-center" onClick={() => onViewApplication(app.job_application_id)}>
                  {app.tailored_resume_json_version ? `v${app.tailored_resume_json_version}` : 'N/A'}
                </td>
                <td className="px-3 py-4 text-sm text-gray-500 dark:text-slate-400 cursor-pointer text-center" onClick={() => onViewApplication(app.job_application_id)}>
                  {companyResumeCounts.get(app.company_id) || 0}
                </td>
                <td className="px-3 py-4 text-sm text-gray-500 dark:text-slate-400 cursor-pointer" onClick={() => onViewApplication(app.job_application_id)}>
                  {app.date_applied && new Date(app.date_applied).getFullYear() > 2000 ? new Date(app.date_applied).toLocaleDateString() : ''}
                </td>
