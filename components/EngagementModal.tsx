...
const contactStatuses = ['To Contact', 'Connection Sent', 'In Conversation', 'Follow-up Needed', 'Not a Fit', 'No Response'];

type ComposerGoal = 'Initial Connection' | 'Follow-up' | 'Reply to Interaction';
...
export const ContactModal = (props: ContactModalProps): React.ReactNode => {
    ...
    const handleGenerate = async () => {
        const prompt = prompts.find(p => p.id === 'networking/strategic-message-gen');
        if (!prompt) {
            setGenerationError('Strategic messaging prompt is not available.');
            return;
        }
        ...
        try {
            ...
            setGeneratedMessages(result.messages.map(m => m.body));
            setGenerationReasoning(result.internal_reasoning);
        } catch (e) {
            setGenerationError(e instanceof Error ? e.message : "Failed to generate message.");
        } finally {
            setIsGenerating(false);
        }
    };
                            ...
                            <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-6 max-h-[80vh] overflow-y-auto p-1">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="p-3 bg-slate-50 dark:bg-slate-900/50 rounded-lg space-y-3 border border-slate-200 dark:border-slate-700">
                                        <div><label className="text-sm font-medium text-slate-600 dark:text-slate-400">Goal</label><select value={composerGoal} onChange={(e) => setComposerGoal(e.target.value as ComposerGoal)} className={`${inputClass} mt-1`}><option>Initial Connection</option><option>Follow-up</option><option>Reply to Interaction</option></select></div>
                                        {composerGoal === 'Reply to Interaction' && <div><label className={labelClass}>Their Post/Comment Text</label><textarea value={interactionDetails} onChange={(e) => setInteractionDetails(e.target.value)} rows={2} className={`${inputClass} mt-1`} placeholder="Paste text of their post or comment..." /></div>}
                                        <div><label className={labelClass}>Your Notes (optional)</label><textarea value={userNotes} onChange={(e) => setUserNotes(e.target.value)} rows={2} className={`${inputClass} mt-1`} placeholder="Guide the AI's tone or angle..." /></div>
                                        ...
                                        {generatedMessages.map((msg, idx) => (
                                            <div key={idx} className="p-3 rounded-md border border-slate-200 dark:border-slate-700 flex flex-col gap-2 bg-white dark:bg-slate-800 shadow-sm transition-shadow hover:shadow-md">
                                                <div className="flex justify-between items-center">
                                                    ...
                                                    <button onClick={() => setComposerText(msg)} className="text-xs font-semibold text-blue-600 dark:text-blue-400 hover:underline flex-shrink-0">
                                                        Use message
                                                    </button>
                                                </div>
                                                <p className="text-sm leading-relaxed text-slate-700 dark:text-slate-300">{msg}</p>
                                            </div>
                                        ))}
                                    </div>
                                    <div>
