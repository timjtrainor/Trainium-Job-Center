...
interface JobDetailsInputStepProps {
  onNext: () => void;
  isLoading: boolean;
  companyName: string;
...
export const JobDetailsInputStep = (props: JobDetailsInputStepProps): React.ReactNode => {
    const { 
        onNext, isLoading, companyName, setCompanyName, jobTitle, setJobTitle, jobLink, setJobLink,
        salary, setSalary, jobDescription, setJobDescription, companyHomepageUrl, setCompanyHomepageUrl
    } = props;
    
    const canProceed = companyName.trim() && jobTitle.trim() && jobDescription.trim();
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div>
        <h2 className="text-xl font-bold text-slate-900 dark:text-white">Step 1: Enter Job Details</h2>
        <p className="mt-1 text-slate-600 dark:text-slate-400">Copy and paste the details from the job posting to get started.</p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
        ...
        <div className="sm:col-span-2">
           <label htmlFor="salary" className="block text-sm font-medium text-slate-700 dark:text-slate-300">Salary (Optional)</label>
