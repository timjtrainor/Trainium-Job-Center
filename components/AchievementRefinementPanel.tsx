...
import { LoadingSpinner, SparklesIcon, XCircleIcon } from './IconComponents';
...
export const AchievementRefinementPanel = ({ isOpen, onClose, achievement, activeNarrative, onSave, jobContext }: AchievementRefinementPanelProps) => {
    ...
    const handleGenerate = async () => {
        await handleAction(async () => {
            ...
            const result = await geminiService.refineAchievementWithContext(context, 'REFINE_ACHIEVEMENT_WITH_CONTEXT');
    ...
    const handleScore = async () => {
        await handleAction(async () => {
            // Removed prompt lookup
            if (!activeNarrative) throw new Error("Active narrative not found.");
            ...
            const result = await geminiService.scoreDualAccomplishments(context, 'SCORE_DUAL_ACHIEVEMENTS');
            setScores(result);
        }, 'score');
    };
    ...
    const handleRefinementChat = async () => {
        ...
        await handleAction(async () => {
            ...
            const result = await geminiService.chatToRefineAchievement(context, 'REFINE_ACHIEVEMENT_CHAT');
